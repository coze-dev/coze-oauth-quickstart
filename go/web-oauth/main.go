...
func loadConfig() (*coze.WebOAuthClient, *coze.OAuthConfig, error) {
	...
	webClient, ok := oauth.(*coze.WebOAuthClient)
	if !ok {
		return nil, nil, fmt.Errorf("invalid OAuth client type: expected Web client")
	}
	return webClient, &oauthConfig, nil
}
